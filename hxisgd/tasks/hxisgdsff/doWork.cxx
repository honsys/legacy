...
/** \file Implementaion of the doWork function for the Astro-H FITS b-table
          to process HXI and SGD First Fits Files. This derives from James Peachey's
...
namespace {
  const string cvsId = "$Name:  $ $Id: doWork.cxx,v 1.12 2012/08/16 17:58:02 dhon Exp $";
} 
...
namespace ahhxisgd {
  ...
  void saveWork(AhFitsFilePtr outfile, int* checkpoint) {
    if( checkpoint )
      ...
      AH_INFO(ahlog::LOW) << " checkpoint: none" << endl;
    return;
  }
  ...
  void doWork(AhFitsFilePtr infile, AhFitsFilePtr outfile, int* checkpoint) {
    if( checkpoint ) {
      if( *checkpoint < 0 ) {
        AH_INFO(ahlog::LOW) << " ?bad checkpoint: " << *checkpoint << endl;
        return;
      }
      AH_INFO(ahlog::LOW) << " checkpoint: " << *checkpoint << endl;
    }
    ...
    if( !valid ) {
      AH_INFO(ahlog::LOW) << " valid instrument name check indicates something is amiss: dohxi, doshd " << dohxi << ", " << dosgd << endl;
      if( !dohxi && !dosgd ) {
    ...
    AH_INFO(ahlog::LOW) << " doInstrum: (checkpoint, names): " << checkpnt << " " << input.name << " " << output.name << endl;
    return;
  } // end doWork
