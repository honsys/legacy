...
namespace {
  const string cvsId = "$Name:  $ $Id: startUp.cxx,v 1.8 2012/07/19 17:09:35 peachey Exp $";
...
namespace ahhxisgd {
  ...
  int sleep(double time) {
    ...
    if( sval < 0 ) {
      AH_INFO(ahlog::LOW) << __PRETTY_FUNCTION__  << " interrupted by signal? " << strerror(errno) << endl;
    }
    return sval;
  } // sleep
  ...
  static void setSigHandler(int* checkpoint) {
    ...
    //for( int i = 0; i < sigcnt; ++i ) {
      ...
      //AH_INFO(ahlog::LOW) << __PRETTY_FUNCTION__  << "> add handler for sig: "<< signame << " == " << signum <<endl;
