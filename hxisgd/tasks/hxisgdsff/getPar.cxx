...
namespace { 
  const string cvsId = "$Name:  $ $Id: getPar.cxx,v 1.12 2012/08/16 17:58:02 dhon Exp $";
}
...
namespace ahhxisgd {
  ...
  void getParAncil(string& ancilfile, int& threadcnt) {
    ...
    ancilfile = "";
    if(  ancilfile != "" )
      AH_INFO(ahlog::LOW) << " ancilfile = " <<  ancilfile << endl;
    else
      AH_INFO(ahlog::LOW) << " no ancilfile input ..." << endl;
    ...
    AH_INFO(ahlog::LOW) << "threadcnt = "<<  threadcnt << endl;
    return;
  }
  ...
  bool getParFITS(string& infile_name, string& outfile_name, string& template_name, bool* pclobber, bool* pprecheck) {
    AH_INFO(ahlog::LOW) << infile_name << ", " << outfile_name << ", " << template_name << endl;
    ...
    if (eOK != status) {
      setStatus(status);
      throw runtime_error(string() + ": problem with templatefile parameter.");
    }
    ...
    if (eOK != status) {
      setStatus(status);
      throw runtime_error(string() + ": problem with infile parameter.");
    }
    ...
    if (eOK != status) {
      setStatus(status);
      throw runtime_error(string() + ": problem with outfile parameter.");
    }
