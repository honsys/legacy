...
namespace {
  const string cvsId = "$Name:  $ $Id: doInstrum.cxx,v 1.8 2012/07/18 16:41:21 dhon Exp $";
...
namespace ahhxisgd {
  ...
  int doInstrum(InstrumWork& input, InstrumWork& output, int* checkpoint) {
    ...
    AH_INFO(ahlog::LOW) <<  __PRETTY_FUNCTION__  << " instrument input : " << input.name << ", output: " << output.name << endl; // "HXI"; // or "SGD";
    if( input.name != output.name ) {
      AH_INFO(ahlog::LOW) <<  __PRETTY_FUNCTION__  << " must be congruent instrument for inpout and output processing ... " <<endl;
      if( checkpoint ) *checkpoint = -1;
      return -1; 
    }
    string instrum = input.name;

    AH_INFO(ahlog::LOW) <<  __PRETTY_FUNCTION__  << " inHdu == " << input.hduIdx << ", inRow: " << input.rowIdx << endl;
    AH_INFO(ahlog::LOW) <<  __PRETTY_FUNCTION__  << " outHdu == " << output.hduIdx << ", outRow: " << output.rowIdx << endl;
    ...
    for( int i = 0; i < inhdus; ++i ) {
      AH_INFO(ahlog::LOW) <<  __PRETTY_FUNCTION__  << " extension HDU name " << i << ": " << in_extnames[i] << endl;
    }
    int outhdus = getHDUnames(output.file, out_extnames);
    for( int i = 0; i < outhdus; ++i ) {
      AH_INFO(ahlog::LOW) <<  __PRETTY_FUNCTION__  << " extension HDU name " << i << ": " << out_extnames[i] << endl;
    }
    ...
    for( int i = 0; i < incolcnt; ++i ) {
      ...
      AH_INFO(ahlog::LOW) <<  __PRETTY_FUNCTION__  << " input column name and number: " << name << " ==> " << colnum << endl;
    }
    ...
    for( int i = 0; i < outcolcnt; ++i ) {
      ...
      AH_INFO(ahlog::LOW) <<  __PRETTY_FUNCTION__  << " output column name and number: " << name << " ==> " << colnum << endl;
    }
    ...
    if( "HXI" == instrum ) {
      ...
      AH_INFO(ahlog::LOW) <<  __PRETTY_FUNCTION__  << " only support Astro-H HXI or SGD ..." << endl;
      if( checkpoint ) *checkpoint = -1;
      return -1; 
    }
