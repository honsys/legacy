...
#define DOHXITEST_C const char doHXItest_rcsId_svnId[] = "$Id: doHXItest.cxx,v 1.19 2012/08/17 20:47:29 dhon Exp $"; 
...
namespace {
  const string cvsId = "$Name:  $ $Id: doHXItest.cxx,v 1.19 2012/08/17 20:47:29 dhon Exp $";
...
namespace ahhxi {
  ...
  int hxiTestInputColCells(InstrumWork& ffwork, HXIFF& hxiff, int hitcnt= 7, int occurId= 77) {
    ...
    HXISF hxisf; // an empty HXI SFF HDU data buff struct
    //
    ...
    int outcols = hxiSimOutputColCells(ffwork.col_names, ffwork.col_sizes, hxisf, chanCounts, chanFlags, pha, adc, occurId);
    // 
    // hxiff struct is extended by hxisff (sff is superset of ff) 
    hxiff = (HXIFF& ) hxisf; 
    return outcols-10;
  }
